...
// Use unified cache for color transform data with automatic cleanup
function getColorRegex(): RegExp {
...
export const color_transform = {
  ...
  colorToRGB(color: string): number[] {
    try {
      ...
      // Check cache first
      if (cache.colorTransformCache.has(normalizedColor)) {
        const cachedResult = cache.colorTransformCache.get(normalizedColor);
        if (cachedResult) {
      ...
      cache.colorTransformCache.set(normalizedColor, JSON.stringify(rgb));
      return rgb;
    } catch (error) {
  ...
  HexToRGB(hex: string): string {
    ...
    if (![3, 4, 6, 8].includes(length)) {
      console_log.consoleLog('error', { hexToRGBError: 'Invalid hex code', hex });
  ...
  HWBToRGB(hwb: string): string {
    ...
    switch (i % 6) {
      case 0: r = v; g = n; bl = w; break;
      case 1: r = n; g = v; bl = w; break;
      case 2: r = w; g = v; bl = n; break;
      case 3: r = w; g = n; bl = v; break;
      case 4: r = n; g = w; bl = v; break;
      case 5: default: r = v; g = w; bl = n; break;
    }
  ...
  separateColor4Transform(value: string): RegExpMatchArray | null {
    ...
    return value.match(getColorRegex());
  },
};
