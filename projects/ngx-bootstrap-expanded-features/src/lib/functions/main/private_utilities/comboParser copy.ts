...
export const comboParser = (
  ...
  // Early return if combo doesn't exist
  if (combIndex === -1 || !values.combos[comb]) {
    multiLog([
      [false, 'valid combo found'],
  ...
  comboTemplates.forEach((originalC: string, templateIndex: number) => {
    ...
    if (c.startsWith(indicatorClass)) {
      ...
      const combCreatedKey: string = existingComboKey || 
        (values.encryptComboCharacters + combosCreatedKeysSet.size);
      ...
      if (cSplit.length > 1) {
        const relevantPseudos = pseudos.filter((p: any) => 
          cSplit[1] && cSplit[1].includes(p.mask)
        );
        ...
        if (relevantPseudos.length > 0) {
          ...
          const sortedPseudos = relevantPseudos.sort((p1: any, p2: any) => 
            c.indexOf(p1.mask) - c.indexOf(p2.mask)
          );
